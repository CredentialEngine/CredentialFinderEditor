	...
	public class Entity_ProcessProfileManager : BaseFactory
	{
		...
		public bool Save( ThisEntity entity, Guid parentUid, int userId, ref List<string> messages )
		{
			...
			using ( var context = new Data.CTIEntities() )
			{
				...
				{
					messages.Add( "Process Profile was invalid. " + SetEntitySummary( entity ) );
					return false;
				}
				if ( isEmpty )
				{
				...
				using ( var context = new Data.CTIEntities() )
				{
					...
					if ( results != null && results.Count > 0 )
					{
						foreach ( DBentity item in results )
						{
							...
							ToMap( item, entity, true, getForProfileLink );
							list.Add( entity );
						}
					}
				}
		...
		public static void ToMap( DBentity from, ThisEntity to, bool includingItems, bool getForProfileLink )
		{
			to.Id = from.Id;
			to.RowId = from.RowId;
			...
			if ( includingItems )
			{
				...
				to.TargetAssessment = Entity_AssessmentManager.GetAll( to.RowId, false );
