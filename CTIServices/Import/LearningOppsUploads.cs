...
{
    public class LearningOppsUpload : BaseUpload
    {
        #region properties
        //identifier, org ctid, name, type, desc, swp
        static int RequiredNbrOfColumns = 6;

        public LoppImportRequest importHelper = new LoppImportRequest();
        List<ImportModel> importList = new List<ImportModel>();
        //
        public ImportMgr importMgr = new ImportMgr();
        public string prevCipList = "";
        //
        string previousLearningMethodTypes = "";
        Enumeration prevLearningMethodType = new Enumeration();
        Enumeration thisLearningMethodType = new Enumeration();
        //
        ...
        public bool UploadFromText(string inputText, string action, AppUser user, string owningOrganizationRowID, ref int owningOrgId, ref List<string> messages)
        {
            ...
            try
            {
                ...
                LoggingHelper.WriteLogFile(5, "LearningOppsUpload.json", jsonList, "", false);
            }
            catch (Exception ex)
            {
                LoggingHelper.LogError(ex, "LearningOppsUpload.UploadFromText Issue with serializing importList error.");
            ...
            try
            {
                ...
                LoggingHelper.DoTrace(5, "LearningOppsUpload.UploadFromText(). " + stats);
            ...
            {
                LoggingHelper.LogError(ex, "LearningOppsUpload.Upload Unexpected error.");
                messages.Add(ex.Message);
            }
        ...
        public bool HandleRecord(int rowNbr, CsvReader csv, ImportModel entity, AppUser user, string owningOrganizationRowID, ref List<string> messages)
        {
            ...
            //NOTE: currently don't have a means for identifying partials. This method will attempt to set isPartialUpdate true if learning opportunity exists.
            //CheckIdentifiers(rowNbr, csv, entity, user, ref messages, ref isPartialUpdate);

            entity.Name = Assign(rowNbr, csv, importHelper.NameHdr, "learning opportunity Name", ref messages, "", true);
			...
			{
                entity.Description = Assign(rowNbr, csv, importHelper.DescHdr, "learning opportunity Description", ref messages, "", (isPartialUpdate ? false : true), MinimumDescriptionLength);
                entity.SubjectWebpage = AssignUrl(rowNbr, csv, importHelper.SubjectWebpageHdr, "learning opportunity Subject webpage", ref messages, "", (isPartialUpdate ? false : true));
                ...
                //               
                entity.LearningMethodTypeList = AssignEnumeration(rowNbr, csv, user, importHelper.LearningMethodTypeHdr, "LearningMethodType", CodesManager.PROPERTY_CATEGORY_Learning_Method_Type, ref prevLearningMethodType, ref thisLearningMethodType, ref previousLearningMethodTypes, ref messages);
                if (!string.IsNullOrWhiteSpace(entity.LearningMethodTypeList))
                {
                    if (entity.LearningMethodTypeList != DELETE_ME)
                        entity.LearningMethodType = thisLearningMethodType;
                }
                ...
                if (importHelper.TeachesCompetencyFrameworkHdr > -1)
                {
                    var competencyFramework = AssignProperty(rowNbr, csv, importHelper.TeachesCompetencyFrameworkHdr, "Target Competency Framework", ref messages);
                    if ( competencyFramework != DELETE_ME )
                    {
                        AssignCompetencyFramework( rowNbr, csv, importHelper.TeachesCompetencyFrameworkHdr, entity, "Target Competency Framework", ref messages );
                    }
                    else
                    {
				...
				//==========================================
				//entity.NaicsList = AssignList( rowNbr, csv, importHelper.NaicsListHdr, "Naics", ref messages );
				//foreach ( var code in entity.NaicsList )
				//{
				//	List<CodeItem> list = CodesManager.NAICS_Search( code );
                ...
                if (importHelper.CreditHourTypeHdr > -1)
                {
                    ...
                    entity.CreditHourType = Assign(rowNbr, csv, importHelper.CreditHourTypeHdr, "CreditHourType", ref messages, "", false);
                }
			...
			if ( importHelper.HasConditionProfile )
				AssignConditionProfile( rowNbr, csv, entity, 7, user, importHelper, ref messages );

			//connections
			if ( importHelper.HasConnectionProfile )
				AssignConnectionProfile( rowNbr, csv, entity, 7, user, importHelper, ref messages );


			//AssignCosts
			if ( importHelper.HasCosts )
			{
				AssignCosts( rowNbr, csv, entity, user, importHelper, entity.FoundExistingRecord, 7, entity.ExistingRecord.RowId, ref messages );
        ...
        private void CheckIdentifiers(int rowNbr, CsvReader csv, ImportModel entity, AppUser user, ref List<string> messages)
        {
			...
			string entityType = "Learning Opportunity";
			...
			if (importHelper.ActionHdr > -1)
            {
                ...
                {
                    messages.Add(string.Format("Row: {0} Invalid Action property: {1}. Valid values are: Add, Update, Append (used for additional rows for the primary learning opportunity row).", rowNbr, entity.ExternalIdentifier));
                }
                else
                {
			...
			if (importHelper.ExternalIdentifierHdr > -1)
            {
                ...
                if (importHelper.CtidHdr > -1)
                    isRequired = false;
                entity.ExternalIdentifier = Assign(rowNbr, csv, importHelper.ExternalIdentifierHdr, "learning opportunity Identifier", ref messages, "", isRequired);
                ...
                else if (!string.IsNullOrWhiteSpace(entity.ExternalIdentifier))
                {
                    ...
                    else
                    {
                        ...
                        {
                            //can only have append, if there is a preceding New/Update
                            messages.Add(string.Format("Row: {0} Error the current record has an Action of 'Append', but the External Identifier: {1}, has not been used in a previous row. Append can only be used with a previous row that is for a full learning opportunity.", rowNbr, entity.ExternalIdentifier));
                        }
                        else
                        {
            ...
            if (importHelper.CtidHdr > -1)
            {
				...
				if ( !string.IsNullOrWhiteSpace( entity.ExternalIdentifier ) )
					isRequired = false;
				entity.CTID = Assign(rowNbr, csv, importHelper.CtidHdr, "learning opportunity CTID", ref messages, "", isRequired);
                if (!string.IsNullOrWhiteSpace(entity.CTID))
                {
                    ...
                    {
                        //can only have append, if there is a preceding New/Update
                        messages.Add(string.Format("Row: {0} Error the current record has an Action of 'Append', but the CTID: {1}, has not been used in a previous row. Append can only be used with a previous row that is for a full learning opportunity.", rowNbr, entity.CTID));
                    }
                    else
                    {
            ...
            if (!string.IsNullOrWhiteSpace(entity.OwningOrganizationCtid))
            {
                if (hasCtid)
                {
                    ...
                    if (entity.FoundExistingRecord)
                    {
                        ...
                        entity.ExistingParentTypeId = CodesManager.ENTITY_TYPE_LEARNING_OPP_PROFILE;
                        if (entity.ExistingRecord.OwningAgentUid != entity.OwningAgentUid)
                        {
                            messages.Add(string.Format("Row: {0} Error a learning opportunity CTID was provided for an existing learning opportunity, and it has a different owning organization: '{1}' than that designated for this upload {2}.", rowNbr, entity.ExistingRecord.OrganizationName, entity.OrganizationName));
                        }
                    }
                }
                else //must have unique id to get here
                {
                    ...
                    //Warning: if learning opportunity is copied, then combination of unique Id, and org Uid will not exist, but may be invalid. This is more likely in the test environment. 
        ...
        private void CheckTargetCredentials(int rowNbr, CsvReader csv, ImportModel entity, AppUser user, ref List<string> messages, ref bool isPartialUpdate)
        {
            if (importHelper.TargetCredentialListHdr == -1)
            {
                ...
                messages.Add(string.Format("Row: {0} One or more target credentials must be included with a learning opportunity.", rowNbr));
                return;
            } //
            ...
            else
            {
                ...
                {
                    messages.Add(string.Format("Row: {0}. Credential Condition Type : Invalid delete request for a condition profile. The condition profile for a credential cannot be deleted from a learning opportunity: {1}", rowNbr, conditionType));
                }
                else
                {
                        ...
                        if (ic != null && ic.Id > 0)
                        {
							...
							if ( ic.OwningAgentUid != entity.OwningAgentUid )
							{
								...
								if ( CanReferenceCredentialFromDifferentOrg )
									warnings.Add( string.Format( "Row: {0}. WARNING. The owner ('{1}') for the target credential ('{2}') is different than the owner selected for this learning opportunity. This is not a fully supported scenario", rowNbr, ic.OwningOrganization.Name, ic.Name ) );
								else
									messages.Add( string.Format( "Row: {0}. ERROR. The owner ('{1}') for the target credential ('{2}') is different than the owner selected for this learning opportunity. This is not a supported scenario at this time.", rowNbr, ic.OwningOrganization.Name, ic.Name ) );
							}
                ...
                if (resultUrl.ToLower() == url.ToLower())
                {
                    if (entity.FoundExistingRecord && entity.ExistingRecord.Id == item.Id)
                    {
                        ...
                        if (item.OwningAgentUid.ToString().ToLower() != entity.OwningAgentUid.ToString().ToLower())
                        {
                            ...
                            {
                                messages.Add(string.Format("Row: {0}. Issue: Attempting to change the owning organization for an existing learning opportunity. The entered learning opportunity already exists with an owning organization of '{1}'. You do not have update rights for the latter organization and so the system cannot allow you to change the owning organization to: '{2}'. ", rowNbr, item.OwningOrganization.Name, defaultOwningOrg.Name));
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            if (isProduction)
                                messages.Add(string.Format("Row: {0}. The subject webpage for this learning opportunity already exists for learning opportunity: '{1}' (#{2}), with an owning organization of '{3}' . This appears to be an invalid scenario. ", rowNbr, item.Name, item.Id, item.OwningOrganization.Name));
                            else
                                warnings.Add(string.Format("Row: {0}. WARNING. The subject webpage for this learning opportunity already exists for learning opportunity: '{1}' (#{2}), with an owning organization of '{3}' . This appears to be an invalid scenario. ", rowNbr, item.Name, item.Id, item.OwningOrganization.Name));
                        }
                        else
                        {
                            ...
                            {
                                if (isProduction)
                                    messages.Add(string.Format("Row: {0}. The subject webpage for this learning opportunity already exists for learning opportunity with the same name: '{1}' (#{2}). This appears to be a duplicate. ", rowNbr, item.Name, item.Id));
                                else
                                    warnings.Add(string.Format("Row: {0}. WARNING. The subject webpage for this learning opportunity already exists for learning opportunity with the same name: '{1}' (#{2}). This appears to be a duplicate. ", rowNbr, item.Name, item.Id));
                            }
                            //else
                            //    messages.Add( string.Format( "Row: {0}. The subject webpage for this learning opportunity already exists for learning opportunity: '{1}' (#{2}). This appears to be a duplicate. ", rowNbr, item.Name, item.Id ) );
                        }
                    }
                    break;
                }
        ...
        public bool ValidateHeaders(string[] headers, bool doingMinimumRequiredChecks, ref List<string> messages)
        {
            ...
            try
            {
                    ...
                    else if (colname.StartsWith("connection"))
                    {
                        ...
                        switch (colname)
                        {
                            ...
                            case "connectionidentifier":
                                ...
                                //importRequest.HasConnectionProfile = true;
                                break;
                        ...
                        switch (colname)
                        {
                            ...
                            case "deliverytypedescription":
                                ...
                                break;
                            case "learningmethodtype":
                                importHelper.LearningMethodTypeHdr = cntr;
                                break;
                            case "language":
                            ...
                            case "keyword":
                                ...
                                break;
                            case "teachescompetencyframeworks":
                            case "teachescompetencyframework":
                                importHelper.TeachesCompetencyFrameworkHdr = cntr;
                                break;
                            case "subjects":
                            case "subject":
                ...
                if (importHelper.NameHdr == -1)
                    messages.Add("Error - a learning opportunity name column must be provided");

                //TBD: 
                if (importHelper.TargetCredentialListHdr == -1)
                {
                    messages.Add("Error - Target Credential(s) must be included with learning opportunities.");
                }

                if (!doingMinimumRequiredChecks)
                {
                    //will have to defer these checks - only if a new learning opportunity
                    if (importHelper.DescHdr == -1)
                        messages.Add("Error - A learning opportunity description column must be provided");

                    if (importHelper.SubjectWebpageHdr == -1)
                        messages.Add("Error - A learning opportunity subject webpage column must be provided");
